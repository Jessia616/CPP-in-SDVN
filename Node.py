...
class Node:
    ...
    def receive_flow(self, flow_reply):
        ...
        if flow_reply.des_id == self.node_id:
            self.routing_table.append(Pkt.RoutingTable(flow_reply.source_id, flow_reply.des_id, flow_reply.des_id, 0, flow_reply.node_id, flow_reply.seq, 0))
            return
            ...
            for table in self.routing_table[::-1]:
                ...
                if pkt.seq == table.seq and pkt.node_id == table.node_id:
                    ...
                    if d < pow(Gp.com_dis, 2):
                        ...
                        # 下一跳节点收分组
                            node_list[table.next_hop_id].receive_pkt(pkt, node_list, controller,d)
                            # 改变路由条目与分组状态以删除
            ...
            for table in self.routing_table[::-1]:
                ...
                if pkt.seq == table.seq and pkt.node_id == table.node_id:
                    d = pow(node_list[table.next_hop_id].position[0] - self.position[0], 2+ self.velocity[0]*0.3) + \
                        pow(node_list[table.next_hop_id].position[1] - self.position[1], 2+ self.velocity[1]*0.3)#(table.dur+pkt.delay) )
                    # 转发成功与否判断
                    if d < pow(Gp.com_dis, 2):
                        # 时延计算
                        # 下一跳节点收分组
