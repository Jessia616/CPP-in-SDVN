...
def f_cal(v_load,vd_load):
    ...
    for i in range(len(vd_load)):
        f[i] = 0.67 * (1-((20 - v_load[i]))/20) + 0.33 *(10 - vd_load[i])
    return f
...
def uua(i,n,s,on_controller,v_c,v_c_d,vehicle_position,v_load,vd_load):
    if len(n) == 0:
        ...
        atleast = 1
        time = 0
        while (change == 1 and atleast == 1) and time < 1000:
            time += 1
            change = 0
            atleast = 0
...
def nzsg(on_s,vehicle_position, controller_position,fix_controller_position):
    ...
    for i in on:
        for j in range(len(controller_position)):
            if i < len(offn):
                if offn[i] == controller_position[j] or controller_position[j][1]<1000 or controller_position[j][0]<1000:
                    on_s[j] = 1
