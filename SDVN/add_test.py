...
##(*^‧^*) 第一个目标函数  能量 参数是车辆位置，需确定的控制器位置，固定开启的控制器位置，开关情况，所有开启控制器的统治车辆数量，车辆到最近控制器距离，所有车辆到最近控制器跳数
def function1 (vehicle_position, controller_position,fix_controller_position, food,on_num,v_c_d, vhop):
    ## ｀（*^﹏^*）根据总跳数 计算发送能量与接受能量之和 单位为 mJ
    sum_hop = np.sum(vhop)
    et = 11.95 * sum_hop

    ## ｀（*^﹏^*）根据总发包数和开启控制器数 计算总控制器开启能量之和 单位为 J
    p = len(vehicle_position)/20/len(on_num)
    ec = 0.05 + 9.95*(2*p-p*p)*len(on_num)
...
def function2 (vehicle_position, controller_position,fix_controller_position, food,on_controller,on_num,v_c,v_c_d):
    ...
    ## 度数负载， 简化为所有控制器的度数乘以车辆到其的距离，其乘积归一化后的方差
    on_num = np.array(on_num)
...
## (≥◇≤)csmaca时延
def tcontca(tprop):
    a = 29.799999999999997
    sum = 0.4169999999999999
    return  a + sum * 2*tprop
## (≥◇≤)csmacd时延
def tcontcd(tprop):
    ...
    a = 34.84883455999999
    sum = 0.4169999999999999*gp.ifg

    return a+sum * (2*tprop-gp.ifg)
...
## p( ^ O ^ )q第三个目标函数 时延
def function3 (vehicle_position, controller_position ,fix_controller_position,food,on_controller,on_num,v_c,v_c_d,vhop):
    ## p( ^ O ^ )q传输时延
    ttrans = 224+ 0.0023629942501200486+29.799999999999997
    ## ((o(^_ ^)o))扩散时延
    tprop = 5
    ##( # ^.^ # )  排队时延
    tqueue = 0.0023629942501200486
    ## (≥◇≤)竞争时延CA 前半部分
    cahop = 29.799999999999997
    ## (≥◇≤)竞争时延CD 前半部分
    cdhop = 30.845634559999993

    ## 车辆传输时延计算
    ## 传输时延，排队时延，竞争时延CA的前半部分，与跳数有关
    sum_hop = np.sum(vhop)
    det = (ttrans+tqueue+cahop) * sum_hop
    ## 扩散时延，竞争时延CA的后半部分，与距离有关
    sum_dis = np.sum(v_c_d)
    det += (sum_dis)/1000 * tprop + (sum_dis)/1000 * tprop *2*0.4169999999999999
    ## 总时延，除以车辆数量归一化
    det = det / len(v_c)
    ...
    ## 计算控制器间跳数之和，小于1000的入选，并乘以所统治车辆数，所统治车辆越多，越有可能进行转发任务
    e = (D<1000)
    e = e.astype(int)
    summ = e.sum(axis=0)
    cdhop = 30.845634559999993
    ## 传输时延，排队时延，竞争时延CD的前半部分，与跳数有关
    dec = (ttrans + tqueue + cdhop) * summ
    on_num1 = on_num/np.max(on_num)
    dec = dec * on_num1

    ## 计算控制器间转发距离之和
    aa = e * D
    summm = aa.sum(axis=0)
    ## 扩散时延，竞争时延CD的后半部分，与距离有关
    dec1 = (summm)/1000 * tprop + (summm)/1000 * tprop *2*0.4169999999999999
    dec1 = dec1 * on_num1

    ## 两部分时延相加，除以控制器数量以归一化
    dec = dec.sum() + dec1.sum()
    dec = dec / len(on_controller)
...
def new_old_sol_compare(Foods, re_list,dim, size,vehicle_position, controller_position,fix_controller_position):
    for num in range(len(Foods)):
        ...
        vhop += 1
        re_list[num][0] = function1(vehicle_position, controller_position,fix_controller_position,Foods[num],on_num,v_c_d,vhop)
        ...
        re_list[num][2] = function3(vehicle_position, controller_position,fix_controller_position,Foods[num],on_controller,on_num,v_c,v_c_d,vhop)
        Foods[num][dim] = 6
        Foods[num][dim + 1] = 0
...
def cal(Foods, re_list,dim,vehicle_position, controller_position,fix_controller_position):
    for num in range(len(Foods)):
        ...
        vhop = vhop.astype(np.int)
        vhop += 1
        re_list[num][0] = function1(vehicle_position, controller_position, fix_controller_position, Foods[num], on_num,
                                    v_c_d, vhop)
        ...
        re_list[num][2] = function3(vehicle_position, controller_position, fix_controller_position, Foods[num],
                                    on_controller, on_num, v_c, v_c_d, vhop)
        Foods[num][dim] = 6
        Foods[num][dim + 1] = 0
...
############################################ 雇佣蜂 ###########################################################
        add1 = employed_bees(Foods, size, dim , GlobalParams)
...
########################################## 侦察蜂 ###############################################################
        ...
        if (timess >= maxiter / 4) or (times % 1000 == 0):
            add3 = scout_bee(Foods, dim, pop_size)
            timess = 0
